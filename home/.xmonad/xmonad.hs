...
myStdWorkspaces = zip [xK_1, xK_2, xK_3, xK_4, xK_5, xK_6, xK_7, xK_8, xK_9] $ map show [1 .. 9 :: Int]
myExtraWorkspaces = [(xK_backslash, "Α"), (xK_0, "0"), (xK_apostrophe, "Ψ"), (xK_igrave, "Ω")]
myWorkspaces = [head myExtraWorkspaces] ++ myStdWorkspaces ++ tail myExtraWorkspaces

myWorkspacesFocus = [((mod4Mask, key), (windows $ W.greedyView ws))
                          | (key,ws) <- myWorkspaces]

myWorkspacesShift = [((mod4Mask .|. shiftMask, key), (windows $ W.shift ws))
                          | (key,ws) <- myWorkspaces]
...
myLayoutHook = avoidStruts . smartBorders $ tiled ||| Mirror tiled ||| Full ||| gimpLayout
  where
